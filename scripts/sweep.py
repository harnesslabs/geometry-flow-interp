...
parser.add_argument("--cfg-scale", type=float, nargs="+", default=[2.0, 2.5, 3.0, 3.5])
parser.add_argument("--noise-scale", type=float, nargs="+", default=[1.0, 1.2])
...
def main(args: argparse.Namespace) -> None:
    ...
    combos = list(itertools.product(args.cfg_scale, args.noise_scale))
    total = len(combos)
    results: list[dict[str, float]] = []

    print(f"\nSweeping {total} combinations...")

    for idx, (cfg, noise) in enumerate(combos, 1):
        ...
        with torch.inference_mode(), utils.maybe_autocast(device):
            while remaining > 0:
                ...
                print(
                    f"  [{idx}/{total}] cfg={cfg} noise={noise} — generating samples: {done}/{args.fid_samples}",
                    end="\r",
                )
        print()
        ...
        results.append(row)
        print(
            f"  [{idx}/{total}] cfg={cfg} noise={noise} -> FID={fid:.2f} IS={is_mean:.2f}+/-{is_std:.2f}"
        )
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["cfg_scale", "noise_scale", "fid", "is_mean", "is_std"]
        )
        writer.writeheader()
    ...
    print(f"\n{'cfg_scale':>10} {'noise_scale':>12} {'FID':>8} {'IS':>14}")
    print("-" * 48)
    for r in results:
        print(
            f"{r['cfg_scale']:>10.1f} {r['noise_scale']:>12.1f} {r['fid']:>8.2f} "
